...
function PhysicalSupplierFilterSelects({
  state,
  onFilterClick,
...
export default function PhysicalSupplierHeaderBar({
  limit,
  search,
  state,
  onLimitChange,
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
        <div className="relative flex-1 w-full min-w-0">
          ...
          <Input
            type="search"
            placeholder="Buscar proveedores físicos…"
            value={search}
            onChange={(e) => onSearchChange(e.target.value)}
            className="pl-9 w-full"
            aria-label="Buscar proveedores físicos"
          />
        </div>
          ...
          <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
            ...
            <SheetContent side="right" className="flex w-full flex-col sm:max-w-sm p-0">
              ...
              <div className="flex-1 overflow-y-auto px-6 pb-4">
                <PhysicalSupplierFilterSelects state={state} onFilterClick={onFilterClick} />
              </div>
