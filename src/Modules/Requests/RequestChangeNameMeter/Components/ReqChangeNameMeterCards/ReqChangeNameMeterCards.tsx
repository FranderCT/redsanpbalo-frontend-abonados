...
import { formatChangeNameMeterDate } from "../../utils/changeNameMeterDate";
...
function formatApplicant(req: ReqChangeNameMeter) {
  return `${req.User?.Name ?? ""} ${req.User?.Surname1 ?? ""} ${req.User?.Surname2 ?? ""}`.trim() || "Sin nombre";
}

function ReqChangeNameMeterCard({
  req,
  onEdit,
  onOpenDetails,
  onOpenComments,
...
export default function ReqChangeNameMeterCards({
  data,
  total,
  page,
  pageCount,
  ...
  const [editingRequest, setEditingRequest] = useState<ReqChangeNameMeter | null>(null);
