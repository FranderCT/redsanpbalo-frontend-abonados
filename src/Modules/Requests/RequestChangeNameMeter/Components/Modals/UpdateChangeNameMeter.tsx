...
import {
  ...
  useUpdateCanCommentChangeNameMeter,
  useUpdateReqChangeNameMeter,
} from "../../Hooks/RequestChangeNameMeterHooks";
...
export default function UpdateReqChangeNameMeterStateModal({
  ...
  const handleToggleCanComment = async (event: React.MouseEvent) => {
    ...
    try {
      ...
      console.error(error);
      setCanComment(!newValue);
      toast.error(error?.response?.data?.message || "No se pudo actualizar el permiso de comentarios");
    }
  };
      ...
      <DialogContent className="max-w-md rounded-none border-slate-200 p-0">
        ...
        <div className="space-y-5 px-6 py-5">
          <div className="space-y-2">
            <Label htmlFor="request-change-name-state-select">Estado</Label>
            {isPending ? (
              ...
              <Select value={stateId} onValueChange={setStateId} disabled={busy}>
                <SelectTrigger id="request-change-name-state-select" className="rounded-none">
