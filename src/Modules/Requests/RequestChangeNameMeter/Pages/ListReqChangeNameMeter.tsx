...
import ReqChangeNameMeterCards from "../Components/ReqChangeNameMeterCards/ReqChangeNameMeterCards";
...
function CreateChangeNameMeterAdminView({ onSuccess }: { onSuccess: () => void }) {
  ...
  const form = useForm({
    ...
    validators: {
      onChange: CreateChangeNameMeterRequestSchema,
      onSubmit: CreateChangeNameMeterRequestSchema,
    },
        ...
        <div className="space-y-4">
          ...
          <label className="block cursor-pointer border-2 border-dashed border-slate-300 p-8 text-center transition-all hover:border-[#1789FC] hover:bg-sky-50/30">
            ...
            <input
              ...
              onChange={(event) => handleFileChange(event.target.files, field.handleChange)}
            />
          </label>
            ...
            {(field) => (
              <UserTypeahead
                ...
                onChange={(userId, picked) => {
                  ...
                  form.setFieldValue("NIS", getChangeNameMeterPrimaryNis(picked));
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div className="text-slate-600">
                      ...
                      {Array.isArray(selectedUser.Nis) && selectedUser.Nis.length ? (
                        <>
                          {" "}
                          • NIS disponibles: <span className="font-mono">{selectedUser.Nis.join(", ")}</span>
                        </>
                      ) : null}
                    </div>
                    {selectedUser.Address ? (
                      <div className="text-slate-600">Dirección: {selectedUser.Address}</div>
                    ) : null}
                    {selectedUser.PhoneNumber ? (
                      <div className="text-slate-600">Teléfono: {selectedUser.PhoneNumber}</div>
                    ) : null}
                  </div>
                </div>
              ) : null
          ...
          <form.Field name="Justification">
            {(field) => (
              <div className="grid gap-2">
                <Label htmlFor="request-change-name-admin-justification">
                  Justificación de la solicitud <span className="text-red-500">*</span>
                </Label>
                <Textarea
                  id="request-change-name-admin-justification"
                  autoFocus
                ...
                <p className="text-xs text-slate-500">
                  Proporcione una justificación clara y detallada para la solicitud.
                </p>
              </div>
            )}
          </form.Field>
...
export default function ListReqChangeNameMeter() {
  ...
  const params = useMemo(
    ...
    [page, limit, userName, stateRequestId],
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const inJustification = String(row.Justification ?? "").toLowerCase().includes(txt);
      const fullName = `${row.User?.Name ?? ""} ${row.User?.Surname1 ?? ""} ${row.User?.Surname2 ?? ""}`
        .toLowerCase()
        .trim();
  ...
  useEffect(() => {
    console.log("[Filtros UI] Solicitudes cambio nombre medidor admin", params);
  }, [params]);
        ...
        <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <h1 className="text-2xl font-semibold tracking-tight">
              {viewMode === "create"
                ? "Nueva solicitud de cambio de nombre del medidor"
                : "Solicitudes de cambio de nombre del medidor"}
            </h1>
            <p className="text-sm text-[#091540]/70">
              {viewMode === "create"
                ? "Registre la solicitud y adjunte la documentación requerida."
