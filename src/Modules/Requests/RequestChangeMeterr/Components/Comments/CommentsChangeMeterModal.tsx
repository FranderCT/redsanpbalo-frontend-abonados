    ...
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }}>
      <DialogContent className="flex h-[95vh] max-h-[95vh] max-w-5xl flex-col overflow-hidden rounded-none p-0">
        <DialogHeader className="sr-only">
          <DialogTitle>Comunicación de solicitud de cambio de medidor #{request.Id}</DialogTitle>
          <DialogDescription>
            Revise la información y comentarios relacionados con la solicitud de cambio de medidor.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="border-b border-gray-200 bg-white p-6">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                ...
                <p className="truncate font-medium text-gray-900" title={String(request.NIS)}>{request.NIS}</p>
              </div>
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Correo Electrónico</span>
                <p className="truncate font-medium text-gray-900" title={request.User?.Email}>{request.User?.Email}</p>
              </div>
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Teléfono</span>
                <p className="truncate font-medium text-gray-900" title={request.User?.PhoneNumber}>{request.User?.PhoneNumber}</p>
              </div>
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
