...
import { CreateChangeMeterRequestAbonadoSchema } from "../schemas/CreateChangeMeterRequestAbonadoSchema";
...
export function UserRequestChangeMeter() {
  ...
  const form = useForm({
    ...
    validators: {
      onChange: CreateChangeMeterRequestAbonadoSchema,
      onSubmit: CreateChangeMeterRequestAbonadoSchema,
    },
        ...
        <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <h1 className="text-2xl font-semibold tracking-tight">
              {viewMode === "create" ? "Solicitud de cambio de medidor" : "Mis solicitudes de cambio de medidor"}
            </h1>
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div className="text-slate-600">
                      ...
                      {availableNis.length > 0 ? (
                        <>
                          {" "}
                          • NIS: <span className="font-mono">{availableNis.join(", ")}</span>
                        </>
                      ) : null}
                    </div>
            ...
            <CardContent className="space-y-6">
              <form.Field name="Location">
                {(field) => (
                  <div className="grid gap-2">
                    <Label htmlFor="request-change-meter-location">
                      Ubicación del medidor <span className="text-red-500">*</span>
                    </Label>
                    <Textarea
                      id="request-change-meter-location"
                      autoFocus
              ...
              {availableNis.length <= 1 ? (
                <form.Field name="NIS">
                  {(field) => (
                    <div className="grid gap-2">
                      <Label htmlFor="request-change-meter-nis">NIS</Label>
                      <input
                        id="request-change-meter-nis"
                        type="text"
                        value={
                          availableNis.length === 1
                ...
                <form.Field name="NIS">
                  {(field) => (
                    <div className="grid gap-2">
                      <Label htmlFor="request-change-meter-nis-select">
                        Seleccione NIS <span className="text-red-500">*</span>
                      </Label>
                      <select
                        id="request-change-meter-nis-select"
              ...
              <form.Field name="Justification">
                {(field) => (
                  <div className="grid gap-2">
                    <Label htmlFor="request-change-meter-justification">
                      Justificación <span className="text-red-500">*</span>
                    </Label>
                    <Textarea
                      id="request-change-meter-justification"
                      className="min-h-[180px] rounded-none resize-none"
                      placeholder="Describa el motivo por el cual solicita el cambio de medidor."
                      value={field.state.value}
                      onChange={(event) => field.handleChange(event.target.value)}
                    />
                    {field.state.meta.isTouched && field.state.meta.errors.length > 0 ? (
