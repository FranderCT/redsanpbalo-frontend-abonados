...
import {
  ...
  useUpdateCanCommentSupervisionMeter,
  useUpdateReqSupervisionMeter,
} from "../Hooks/ReqSupervisionMeterHooks";
...
export default function UpdateReqSupervisionMeterStateModal({
  ...
  const updateCanCommentMutation = useUpdateCanCommentSupervisionMeter();
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message ||
          "No se pudo actualizar el permiso de comentarios",
      );
    }
  };
        ...
        <div className="space-y-5 px-6 py-5">
          <div className="space-y-1 text-sm text-slate-600">
            ...
            <p>
              ...
              <span className="font-medium text-slate-900">{req.NIS ?? "-"}</span>
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="request-supervision-state-select">Estado</Label>
            {isPending ? (
              ...
              <Select value={stateId} onValueChange={setStateId} disabled={busy}>
                <SelectTrigger
                  id="request-supervision-state-select"
